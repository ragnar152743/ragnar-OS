...
def boot_sequence() -> tuple[MiniOS, BootReport]:
    ...
    steps.append(f"Profiles: {len(list(os_instance.profile_manager.list_profiles()))} loaded")
    steps.append(f"Sandboxed apps: {len(list(os_instance.application_manager.list_applications()))}")
    steps.append(f"Virtual machines: {os_instance.virtualization_manager.status()}")
    steps.append(f"Marketplace entries: {len(os_instance.marketplace.list_entries())}")
